...
VERSION := $(shell git describe --tags --abbrev=0 || echo "UnknownVersion")
GIT_BRANCH := $(shell git rev-parse --abbrev-ref HEAD || echo "UnknownVersion")
GIT_COMMIT := $(shell git rev-parse HEAD || echo "UnknownVersion")
BUILD_TIME := $(shell date -u || echo "UnknownVersion")
LDFLAGS := '-w -s -X "github.com/xmapst/lightsocks.Version=$(VERSION)" \
	-X "github.com/xmapst/lightsocks.GitBranch=$(GIT_BRANCH)" \
	-X "github.com/xmapst/lightsocks.GitCommit=$(GIT_COMMIT)" \
	-X "github.com/xmapst/lightsocks.BuildTime=$(BUILD_TIME)" \
	'
